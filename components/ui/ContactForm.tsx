...
export function ContactForm() {
    ...
    const validate = (): boolean => {
        ...
        if (!formData.nombre.trim()) {
            newErrors.nombre = t.contact.errors.nombre;
        }
        if (!formData.apellido.trim()) {
            newErrors.apellido = t.contact.errors.apellido;
        }
        ...
        if (!formData.idea.trim()) {
            newErrors.idea = t.contact.errors.idea;
        }
    ...
    const buildWhatsAppMessage = (): string => {
        const message = `${t.contact.whatsappGreeting} *${formData.nombre} ${formData.apellido}*.\n\n${t.contact.whatsappEmail}: ${formData.email}\n\n${t.contact.whatsappIdea}:\n${formData.idea}`;
        ...
        <div className="w-full max-w-2xl mx-auto text-left">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <motion.input
                        type="text"
                        placeholder={t.contact.placeholders.nombre}
                        value={formData.nombre}
                        onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                        className={inputClassName}
                        whileFocus={{ scale: 1.01 }}
                        aria-label={t.contact.placeholders.nombre}
                    />
                    {errors.nombre && (
                ...
                <div>
                    <motion.input
                        type="text"
                        placeholder={t.contact.placeholders.apellido}
                        value={formData.apellido}
                        onChange={(e) => setFormData({ ...formData, apellido: e.target.value })}
                        className={inputClassName}
                        whileFocus={{ scale: 1.01 }}
                        aria-label={t.contact.placeholders.apellido}
                    />
                    {errors.apellido && (
            ...
            <div className="mb-6">
                <motion.textarea
                    placeholder={t.contact.placeholders.idea}
                    value={formData.idea}
                    onChange={(e) => {
                    ...
                    whileFocus={{ scale: 1.01 }}
                    aria-label={t.contact.placeholders.idea}
                />
